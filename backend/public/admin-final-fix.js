(() => {
  ...
  const installStyles = () => {
    ...
    style.textContent = `
      ...
      @media(max-width: 850px) {
        ...
        .booking-clock-safe { margin-left: 0; margin-top: 8px; display: flex; width: max-content; }
      }
    `;
    document.head.appendChild(style);
  };

  const addSafeBookingClock = () => {
    ...
    let clock = $('bookingClockSafe');
    if (!clock) {
      clock = document.createElement('span');
      clock.id = 'bookingClockSafe';
      clock.className = 'booking-clock-safe';
      title.insertAdjacentElement('afterend', clock);
    }

    const now = new Date();
    const locale = navigator.language || 'en-NG';
    const dateText = now.toLocaleDateString(locale, { day: '2-digit', month: 'short', year: 'numeric' });
    const timeText = now.toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' });
  ...
  setInterval(addSafeBookingClock, 60000);
  setInterval(() => {
    addDirectMessageBox();
    strengthenEditRoom();
    fixUploadedImageField();
